      ...
      <div className="border-t border-brand-100/50 pt-4 space-y-4">
        ...
        {showInadmissibility && (
          <div className="space-y-4 p-4 rounded-xl border border-brand-100/80 bg-white/50 animate-fade-in">
            ...
            <InadQuestionnaire
              data={data}
              inadMyUscisDetalle={data.inadMyUscisDetalle || ""}
              getError={(field) => err(field as keyof CaseBackgroundData)}
              onFieldChange={(field, value) =>
                handleChange(field as keyof CaseBackgroundData, value)
              }
              onMyUscisChange={(inadMyUscis, inadMyUscisDetalle) =>
                onChange({ inadMyUscis, inadMyUscisDetalle })
              }
            />
          </div>
        )}
